...
export const Navbar: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'Education', href: isHomePage ? '#education' : '/#education' },
    ...
    >
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-white/70 dark:bg-slate-900/70 border border-slate-200/80 dark:border-slate-800 px-3 py-1.5 rounded-full shadow-xs backdrop-blur-md">
          {navItems.map((item) => {
            const isBlogRoute = item.href === '/blog' && pathname.startsWith('/blog');
            const isActive = isBlogRoute || (isHomePage && activeSection === item.label.toLowerCase());
